...
function render_network_acls(config) {
    ...
    let grouped_expanded_network_traffic_rules = _.get(config,'grouped_expanded_network_traffic_rules');
    let availability_zones = _.get(config, 'availability_zones');

    let expanded_network_traffic_rules = _.get(config, 'expanded_network_traffic_rules');
    let network_tiers = _.get(config,'network_tiers');
    let nat_gateway_network_tier = _.get(config,'nat_gateway_network_tier');
    ...
    _.each(grouped_expanded_network_traffic_rules, function (network_tier_group, network_tier_name) {
        ...
        if (_.includes(ephemeral_ingress_ports_rule.cidr_blocks, '"0.0.0.0/0"')) {
        ...
        let ephemeral_egress_ports_rule = {
            name: `${network_tier_name}_ephemeral_egress_to_cidr_blocks`,
            type: 'egress',
            traffic_type_name: 'ephemeral',
        ...
        if (_.includes(ephemeral_egress_ports_rule.cidr_blocks, '"0.0.0.0/0"')) {
        ...
        _.set(network_acls, network_tier_name, network_acl);
    });


    let external_network_rules = {};
    _.each(_.filter(expanded_network_traffic_rules, {source_tier:{public:false},destination_tier:{external:true}}), function(network_traffic_rule){
        _.set(external_network_rules, [network_traffic_rule.traffic_type.name, 'traffic_type'], network_traffic_rule.traffic_type);
        _.set(external_network_rules, [network_traffic_rule.traffic_type.name, 'network_tiers', network_traffic_rule.source_tier.name], network_traffic_rule.source_tier);
    });


    let nat_gateway_network_acl = _.get(network_acls, nat_gateway_network_tier);

    _.each(external_network_rules, function(external_network_rule){

        let traffic_type = external_network_rule.traffic_type;

        let nat_gateway_network_tier_rule = {
            name: `${nat_gateway_network_tier}_${traffic_type.name}_ingress_to_nat_gateway_from_cidr_blocks`,
            type: 'ingress',
            traffic_type_name: traffic_type.name,
            traffic_type: traffic_type,
            cidr_blocks: _.sortBy(_.uniq(_.flatten(_.map(external_network_rule.network_tiers, 'cidr_block')))),
            network_tier_name: nat_gateway_network_tier,
            egress: false,
            port: _.get(traffic_type, 'port'),
            protocol: _.get(traffic_type, 'protocol'),
        };

        nat_gateway_network_acl.ingress_rules.push(nat_gateway_network_tier_rule);
...
function render_nat_gateways(config) {
    let availability_zones = _.get(config, 'availability_zones');
    let public_network_tiers = _.get(config, 'public_network_tiers');
    let private_network_tiers = _.get(config, 'private_network_tiers');
    let nat_gateway_network_tier = _.get(config, 'nat_gateway_network_tier');

    let context = {};
    context.public_network_tiers = public_network_tiers;
    context.private_network_tiers = private_network_tiers;
    context.availability_zones = availability_zones;
    context.nat_gateway_network_tier = nat_gateway_network_tier;
